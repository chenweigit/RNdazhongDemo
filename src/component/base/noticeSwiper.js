...
export default class ScrollVertical extends Component {
    ...
    render() {
        console.log(this.state.translateValue.y);
        console.log(this.state.kb_content);
        return (
    ...
    _startAnimation = () => {
        let {kb_tempValue, scrollHeight, onChange} = this.state;
        let currentScroll = kb_tempValue - scrollHeight;
        
        if (onChange) {
            let index = Math.abs(currentScroll) / scrollHeight;
